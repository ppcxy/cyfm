...
//方位辨别
    Dates.orien = function(obj, pos){
        ...
        //target 结束位置如果小于右边界
        if(rect.left + obj.offsetWidth < document.body.offsetWidth-20){
            //right 重置为auto 避免出现left right同时出现值.
            obj.style.right = "auto";
            //target left 作为日期控件left
            obj.style.left = rect.left + (pos  ?0 : Dates.scroll(1)) + 'px';
        }else{
            //left 重置为auto 避免出现left right同时出现值.
            obj.style.left = "auto";
            //target 右边界 作为日期控件left
            obj.style.right = document.body.offsetWidth-(rect.left+rect.width) - (pos ? 0 : Dates.scroll(1))+'px';
        }
