...
<div class="listTableWrap">
    <table id="contentTable" data-tid="user"
        ...
        <tbody>
        <c:forEach items="${page.content}" var="su">
            <tr>
                <td class="check"><input name="ids" type="checkbox" value="${su.id}"></td>
                <td>${su.user.username}&nbsp;</td>
                <td>${su.user.name}&nbsp;</td>
                <td>${su.user.tel}&nbsp;</td>
                <td>${su.user.email}&nbsp;</td>
                <td>${su.realName}</td>
                <td>${allStatus[su.user.status]}&nbsp;</td>
                    <%-- 根据权限显示按钮 --%>
            </tr>
        </c:forEach>
        </tbody>
