...
<div data-table="table" class="tabbable-line">
    <ul class="nav nav-tabs">
        <li ${empty param['search.show_eq'] ? 'class="active"' : ''}>
            <a href="${ctx}/shop/gs/type/tree?async=${not empty param.async and param.async eq true}">
        ...
        <li ${not empty param['search.show_eq'] ? 'class="active"' : ''}>
            <a href="${ctx}/shop/gs/type/tree?search.show_eq=true&async=${not empty param.async and param.async eq true}">
                <i class="fa fa-table"></i>
                显示的
            </a>
        </li>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        $.zTree.initMovableTree({
            ...
            permission: <cy:treePermission resourceIdentity="gsType"/>,
            autocomplete : {
                enable : true
            },
            setting : {
                callback : {
                    onClick: function(event, treeId, treeNode, clickFlag) {
                        parent.frames['listFrame'].location.href='${ctx}/shop/gs/type/' + treeNode.id + "/update?async=" + async ;
                    }
                }
            }
        });
