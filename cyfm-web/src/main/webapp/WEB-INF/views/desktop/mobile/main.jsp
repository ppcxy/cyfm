...
<head>
	...
	<style>
        ...
        div.mobile-main .nav li .three {
            padding-left: 15px;
            padding-right: 15px;
            display: none;
        }

        div.mobile-main .nav li .three h2{
            width: 57px;
        }

        div.mobile-main .nav li .three li{
            width: 25%;
        }
	...
	<style>
        ...
        .layui-layer-ico{
            background:url(${ctx}/static/plugins/layer-dialog/skin/default/icon.png) no-repeat !important;
        }
	</style>
...
<div class="footer-menu-bar">
	<ul>
		...
		%>
		<li><a href="${ctx}/auth/join?joinSystemName=<%=basePath%>&redirectUrl=/" class="bottom-menu"><i class="fa fa-link icon-large menu-icon"></i></a></li>
	</ul>
</div>
...
<script>
	...
	$(".sub-menu.three").prev("a").click(function () {
		layer.open({
			type: 1,
			fixed: true,
			title: '',
			...
			closeBtn:1,
			shadeClose:true,
			area:['95%',$(this).next(".three").find("li").size()*150+"px"],
			content:$(this).next(".three")
		})
	})
